...
\
	for (int i = 0; i < array->count; ++i) {\
		if (print_func != NULL) {\
			...
			if (same(T, int)) {\
				str = "%d";\
			} else if (same(T, float) || same(T, double)) {\
				str = "%f";\
			} else {\
				printf("\nError: calling array_print(%s) with unsupported type. Specify custom printing function or add more types at Array.c::array_print(T)\n", #T);\
				abort();\
			}\
